...
class PatchEmbed(nn.Module):
    def __init__(self, img_w=256, img_h=128, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # x = batch_size, num_patches+1(position), embed_dim(768)
        B, N, C = x.shape
...
class ViTGeoGuessr(nn.Module):
    def __init__(self, img_w=256, img_h=128, patch_size=16, in_c=3, num_classes=2, embed_dim=768, 
        ...
        self.patch_embed = embed_layer(img_w=img_w, img_h=img_h, patch_size=patch_size, 
